...
## Архитектура: Проект_ЧТЗ_Энергомера

```mermaid
flowchart TD
  A1["Модуль подключения счётчиков"] --> A2["Анализ типа нагрузки"]
  A2 --> A3["Историческое хранилище"]
  A2 --> A4["Интеграция с ceCloud"]
```

## Архитектура: ТЗ_AICapture

```mermaid
flowchart TD
  A1["Модуль видеоприёма"] --> A2["Буфер и синхронизация"]
  A2 --> A3["Маршрутизатор потоков"]
  A2 --> A4["Shared Memory интерфейс"]
```

## Архитектура: ТЗ_Платформа_AIOT

```mermaid
flowchart TD
  A1["Модуль видеозахвата"] --> A2["Реконструкция сцен"]
  A2 --> A3["Распознавание признаков"]
  A3 --> A4["Рекомендательная система"]
  A4 --> A5["ML-хостинг"]
  A5 --> A6["Нейросетевой конфигуратор"]
  A6 --> A7["Управление микросервисами"]
```